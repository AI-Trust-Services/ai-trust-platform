...
async def get_model_card(model_id: str) -> ModelCardResponse:
    async with SessionLocal() as session:
        result = await session.execute(
            select(ModelCard).where(ModelCard.id == model_id)
        )
...
async def create_model_card(body: ModelCardCreate) -> ModelCardResponse:
    ...
    logger.info(
        "model_card.created", extra={"model_id": row.id, "model_name": row.name}
    )
    return ModelCardResponse.model_validate(row)
...
async def update_model_card(model_id: str, body: ModelCardUpdate) -> ModelCardResponse:
    ...
    logger.info(
        "model_card.updated",
        extra={"model_id": model_id, "fields": sorted(updates.keys())},
    )
    return ModelCardResponse.model_validate(row)


@router.delete(
    "/model-cards/{model_id}", dependencies=[Depends(require_permission(SYSTEMS_WRITE))]
)
...
@router.get(
    "/model-cards/{model_id}/systems",
    response_model=list[ModelSystemResponse],
    dependencies=[Depends(require_permission(SYSTEMS_READ))],
)
    ...
    async with SessionLocal() as session:
        ...
        result = await session.execute(
            ...
            .join(
                ...
                AISystem.id == AISystemModelCard.__table__.c.system_id,
            )
            .where(AISystemModelCard.__table__.c.model_card_id == model_id)
            .order_by(AISystem.name)
        )
        return [
            ModelSystemResponse.from_system(sys, role) for sys, role in result.all()
        ]
