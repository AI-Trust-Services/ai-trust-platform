...
Steps:
  ...
  4. If PROVISION_DEV_USERS=true, assign user:admin the platform_admin role.
     Additionally, if INITIAL_ADMIN_USER is set, assign it platform_admin.
...
Optional env vars:
  ...
  PROVISION_DEV_USERS    "true" to seed user:admin as platform_admin (local only)
  INITIAL_ADMIN_USER     username to seed as platform_admin (production)
"""
import asyncio
import os
import sys
...
async def seed_admin_users(client: OpenFgaClient) -> None:
    """Assign platform_admin membership to bootstrap admin users."""
    admins = []
    ...
    tuples = [
        ClientTuple(user=f"user:{name}", relation="member", object="role:platform_admin")
        for name in admins
    ]
    await _write_tuples_idempotent(client, tuples, label="admin assignment")
    print(f"Seeded platform_admin for: {', '.join(admins)}")
