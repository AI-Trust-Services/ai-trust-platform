...
async def test_put_smtp_preserves_password_when_empty_string_provided(
...
async def test_smtp_test_auth_error_returns_descriptive_message(
    client: httpx.AsyncClient,
):
    import aiosmtplib
    ...
    with (
        patch("app.routers.smtp.SessionLocal", return_value=session),
        patch(
            "app.routers.smtp.aiosmtplib.send",
            new=AsyncMock(
...
async def test_smtp_test_connect_error_returns_descriptive_message(
    client: httpx.AsyncClient,
):
    import aiosmtplib
...
async def test_smtp_test_timeout_returns_descriptive_message(client: httpx.AsyncClient):
    ...
    assert r.json()["success"] is False
    assert (
        "timed out" in r.json()["message"].lower()
        or "unreachable" in r.json()["message"].lower()
    )


async def test_smtp_test_smtp_timeout_error_returns_descriptive_message(
    client: httpx.AsyncClient,
):
    import aiosmtplib
