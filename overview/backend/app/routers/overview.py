...
async def get_overview_stats() -> dict:
    async with SessionLocal() as session:
        total = (
            await session.execute(select(func.count()).select_from(AISystem))
        ).scalar_one()

        avg_compliance = float(
            (await session.execute(select(func.avg(AISystem.compliance)))).scalar_one()
            or 0.0
        )
        ...
        lifecycle_rows = (
            await session.execute(
                select(AISystem.lifecycle, func.count().label("n")).group_by(
                    AISystem.lifecycle
                )
            )
        ).all()
        ...
        compliance_by_tier_rows = (
            await session.execute(
                select(
                    AISystem.tier, func.avg(AISystem.compliance).label("avg_c")
                ).group_by(AISystem.tier)
            )
        ).all()
        compliance_by_tier = {
            r.tier: round(float(r.avg_c), 1) for r in compliance_by_tier_rows
        }

        type_rows = (
            await session.execute(
                select(AISystem.system_type, func.count().label("n")).group_by(
        ...
        model_type_rows = (
            await session.execute(
                select(ModelCard.model_type, func.count().label("n")).group_by(
        ...
        model_provider_rows = (
            await session.execute(
                select(ModelCard.provider, func.count().label("n")).group_by(
                    ModelCard.provider
                )
            )
        ).all()
                ...
                select(
                    ...
                    func.sum(
                        case(
                            (
                                and_(
                                    AISystem.compliance >= 20, AISystem.compliance < 40
                                ),
                                1,
                            ),
                            else_=0,
                    ...
                    func.sum(
                        case(
                            (
                                and_(
                                    AISystem.compliance >= 40, AISystem.compliance < 60
                                ),
                                1,
                            ),
                            else_=0,
                    ...
                    func.sum(
                        case(
                            (
                                and_(
                                    AISystem.compliance >= 60, AISystem.compliance < 80
                                ),
                                1,
                            ),
                            else_=0,
        ...
        attention_rows = (
            (
                await session.execute(
                    ...
                    .where(
                        or_(
                            ...
                            and_(
                                ...
                                ~exists(
                                    select(
                                        AISystemModelCard.__table__.c.system_id
                                    ).where(
                                        AISystemModelCard.__table__.c.system_id
                                        == AISystem.id
                                    )
                    ...
                    .order_by(AISystem.compliance.asc())
                    .limit(20)
                )
            )
            .scalars()
        ...
        attention = [
            {
                ...
                "reason": (
                    "Prohibited system"
                    if r.tier == "prohibited"
                    else "High-risk on market with low compliance"
                    if r.tier == "high" and r.compliance < 50
...
async def get_compliance_stats(
    ...
    async with SessionLocal() as session:
        ...
        obl_rows = (
            await session.execute(
                select(Obligation.status, func.count().label("n")).group_by(
                    Obligation.status
                )
            )
        ).all()
        ...
        missing_count = (
            await session.execute(
                ...
                .where(
                    ...
                    ~(
                        select(evidence_requirements.c.evidence_id)
                        .join(
                            Requirement,
                            Requirement.id == evidence_requirements.c.requirement_id,
                        )
                        .join(
                            Evidence, Evidence.id == evidence_requirements.c.evidence_id
                        )
                        .where(
                            Requirement.obligation_id == Obligation.id,
                            Evidence.status == "approved",
                        )
        ...
        fw_rows = (
            await session.execute(
                select(
                    ...
                    func.sum(
                        case((Obligation.status == "fulfilled", 1), else_=0)
                    ).label("fulfilled"),
                )
        ...
        framework_compliance = [
            {
                ...
                "score": round(int(r.fulfilled or 0) / int(r.total) * 100, 1)
                if r.total
                else None,
            }
            for r in fw_rows
        ...
        # ai_system_id is resolved via evidence_requirements → Requirement.ai_system_id
        evd_req = aliased(Requirement)
        ...
        evd_deadlines = (
            await session.execute(
                ...
                .join(evd_req, evd_req.id == evidence_requirements.c.requirement_id)
                .join(evd_sys, evd_sys.id == evd_req.ai_system_id)
                .where(
                    Evidence.validity_until >= today,
                    Evidence.validity_until < window_end,
                    Evidence.status == "approved",
        ...
        heatmap_rows = (
            await session.execute(
                select(
                    AISystem.tier, tier_x, residual_y, func.count().label("n")
        ...
        risk_heatmap = [
            {
                "tier": r.tier,
                "tier_x": r.tier_x,
                "residual_risk_y": r.residual_risk_y,
                "count": r.n,
            }
