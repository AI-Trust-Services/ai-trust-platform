...
export default function ControlPicker({ value, onChange }: Props) {
  ...
  // filter controlCatalog by search term, then sort by control_ref → title
  const filtered = controlCatalog
    .filter((c) => {
      if (!search) return true;
      const q = search.toLowerCase();
      // match on title or control_ref (e.g. "Art.9:risk-management")
      return c.title.toLowerCase().includes(q) || (c.control_ref ?? "").toLowerCase().includes(q);
    })
    .sort((a, b) => {
      // null control_ref sorts last (U+FFFD is the highest unicode sort value)
      const ra = a.control_ref ?? "￿";
      const rb = b.control_ref ?? "￿";
      // primary sort: by obligation ref (e.g. "Art.9")
      if (ra !== rb) return ra.localeCompare(rb);
      // secondary sort: alphabetically by title within the same ref
