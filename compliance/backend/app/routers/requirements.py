...
async def list_requirements(
    ...
    async with SessionLocal() as session:
        ...
        if ai_system_id:
            stmt = stmt.where(
                or_(
                    Requirement.ai_system_id == ai_system_id,
                    Requirement.ai_system_id.is_(None),
                )
            )
        if obligation_id:
            stmt = stmt.join(
...
async def create_requirement(body: RequirementCreate) -> RequirementResponse:
    ...
    logger.info(
        "requirement.created",
        extra={"requirement_id": row.id, "ai_system_id": row.ai_system_id},
    )
...
@router.post(
    "/requirements/{requirement_id}/link/{obligation_id}",
    response_model=RequirementDetailResponse,
    dependencies=[Depends(require_permission(ASSESSMENTS_WRITE))],
)
async def link_obligation(
    requirement_id: str, obligation_id: str
) -> RequirementDetailResponse:
    async with SessionLocal() as session:
    ...
    logger.info(
        "requirement.linked",
        extra={"requirement_id": requirement_id, "obligation_id": obligation_id},
    )
