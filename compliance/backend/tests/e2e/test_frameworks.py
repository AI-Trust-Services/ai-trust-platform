"""E2E tests for GET/PATCH /api/v1/frameworks."""
...
async def test_get_framework_by_id(client: httpx.AsyncClient):
    r = await client.get("/api/v1/frameworks/FRM-EU-AI-ACT")
    assert r.status_code == 200
    body = r.json()
...
async def test_get_framework_404_on_missing(client: httpx.AsyncClient):
    r = await client.get("/api/v1/frameworks/FRM-UNKNOWN")
    assert r.status_code == 404


async def test_disable_framework(client: httpx.AsyncClient):
    r = await client.patch("/api/v1/frameworks/FRM-ISO-42001", json={"enabled": False})
    assert r.status_code == 200
...
async def test_enable_framework(client: httpx.AsyncClient):
    ...
    r = await client.patch("/api/v1/frameworks/FRM-ISO-42001", json={"enabled": True})
    assert r.status_code == 200
    assert r.json()["enabled"] is True


async def test_patch_framework_404_on_missing(client: httpx.AsyncClient):
    r = await client.patch("/api/v1/frameworks/FRM-UNKNOWN", json={"enabled": False})
    assert r.status_code == 404
