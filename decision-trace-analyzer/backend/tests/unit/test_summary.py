...
class TestClassifyKind:
    ...
    def test_other_default(self):
        assert (
            classify_kind(make_span(operation_name="", span_name="parser_run"))
            == "other"
        )
...
class TestExtractGoal:
    def test_root_input_user_message(self):
        root = make_llm_span(
            user_prompt="What's the weather in Berlin?", assistant_reply="Hi"
        )
...
class TestExtractFinalAnswer:
    ...
    def test_fallback_to_last_llm(self):
        ...
        early_llm = make_llm_span(
            span_id="llm-1",
            parent_span_id="root",
            started_offset_ms=10,
            assistant_reply="thinking…",
        )
        later_llm = make_llm_span(
            span_id="llm-2",
            parent_span_id="root",
            started_offset_ms=200,
            assistant_reply="Final answer.",
        )
...
class TestClassifyOutcome:
    ...
    def test_errored_when_llm_failed_no_status_message(self):
        ...
        llm = make_llm_span(
            span_id="llm", parent_span_id="root", status_code=2, status_message=""
        )
    ...
    def test_refused_via_explicit_guardrail_attribute(self):
        root = make_llm_span(
            span_id="root",
            parent_span_id="",
            user_prompt="something bad",
            assistant_reply=None,
        )
        gr = make_guardrail_span(
            span_id="gr",
            parent_span_id="root",
            triggered_attr=True,
            status_code=0,
    ...
    def test_refused_via_heuristic_status_error(self):
        ...
        root = make_llm_span(
            span_id="root", parent_span_id="", user_prompt="x", assistant_reply=None
        )
        gr = make_guardrail_span(
            span_id="gr",
            parent_span_id="root",
    ...
    def test_partial_when_tool_errored_but_trace_continued(self):
        ...
        bad_tool = make_tool_span(
            span_id="t", parent_span_id="root", tool_name="get_weather", status_code=2
        )
    ...
    def test_answered_when_tool_errored_but_trace_recovered(self):
        ...
        root = make_llm_span(
            ...
            parent_span_id="",
            user_prompt="hi",
            assistant_reply="best-effort answer",
            finish_reasons="stop",
        )
        bad_tool = make_tool_span(
            span_id="t", parent_span_id="root", tool_name="get_weather", status_code=2
    ...
    def test_errored_takes_precedence_over_refused(self):
        ...
        root = make_llm_span(
            status_code=2, status_message="boom", user_prompt="x", assistant_reply=None
        )
...
class TestAggregateTools:
    ...
    def test_span_name_fallback_strips_op_prefix(self):
        ...
        spans = [
            make_tool_span(
                tool_name=None, fallback_span_name="execute_tool get_weather"
            )
        ]
        result = aggregate_tools(spans)
        assert result[0].name == "get_weather"
...
class TestAggregateRetrievals:
    ...
    def test_span_name_fallback(self):
        spans = [
            make_retriever_span(db_collection=None, fallback_span_name="vector_search")
        ]
        result = aggregate_retrievals(spans)
...
class TestBuildSummary:
    ...
    def test_empty_content_yields_nulls_but_full_record(self):
        ...
        ret = make_retriever_span(
            span_id="r", parent_span_id="root", db_collection="kb", started_offset_ms=10
        )
...
class TestExtractDecisionPath:
    ...
    def test_skips_plumbing(self):
        ...
        plumb2 = make_span(
            ...
            started_offset_ms=5,
            operation_name="",
            span_name="RunnableAssign<scratchpad>",
        )
        llm = make_llm_span(
            span_id="l",
            parent_span_id="p",
...
class TestFlags:
    ...
    def test_tool_failure_lists_failed_tools(self):
        ...
        bad = make_tool_span(
            span_id="t1", parent_span_id="root", tool_name="get_weather", status_code=2
        )
    ...
    def test_retry_loop_when_same_tool_fires_four_times(self):
        root = make_llm_span(
            span_id="root", parent_span_id="", user_prompt="x", assistant_reply="y"
        )
        tools = [
            make_tool_span(
                span_id=f"t{i}",
    ...
    def test_truncated_output_flag_on_length_finish_reason(self):
        ...
        flags = compute_flags(
            [root], goal="long story", final_answer="Once upon a time…"
        )
