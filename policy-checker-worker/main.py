...
async def eval_avg_compliance_below(rule: AlertRule, ch) -> tuple[bool, float]:
    async with SessionLocal() as session:
        avg = (
            await session.execute(select(func.avg(AISystem.compliance)))
        ).scalar_one()
    if avg is None:
        return False, 0.0
    avg = float(avg)
...
async def eval_high_risk_on_market_low_compliance(
    rule: AlertRule, ch
) -> list[EvalResult]:
...
async def eval_high_latency(rule: AlertRule, ch) -> list[EvalResult]:
    ...
    for r in rows:
        ...
        if service not in system_map:
            log.warning(
                "alert_worker.unregistered_service", extra={"service_name": service}
            )
            continue
...
async def eval_market_system_no_model_card(rule: AlertRule, ch) -> list[EvalResult]:
    ...
    async with SessionLocal() as session:
        ...
        has_model = exists(
            select(AISystemModelCard.__table__.c.system_id).where(
                AISystemModelCard.__table__.c.system_id == AISystem.id
            )
        ).correlate(AISystem)
    ...
    for sid, name, is_missing in rows:
        results.append(
            EvalResult(
                ...
                description=f"System on market without a model card: {name} ({sid})"
                if is_missing
                else "",
                entity_id=sid,
                entity_type="ai_system",
    ...
    for sid, name, compliance in rows:
        ...
        results.append(
            EvalResult(
                triggered=no_score,
                value=float(compliance),
                description=f"GPAI system with no compliance score: {name} ({sid})"
                if no_score
                else "",
                entity_id=sid,
                entity_type="ai_system",
    ...
    async with SessionLocal() as session:
        ...
        for row in rows:
            ...
            baseline_row = (
                await session.execute(
                    text(
                        "SELECT model_name FROM service_model_baselines WHERE service_name = :svc"
                    ),
                    {"svc": system_id},
                )
            ).fetchone()
            ...
            if baseline_row is None:
                ...
                await session.execute(
                    text(
                        "UPDATE service_model_baselines SET last_seen_at = :ts WHERE service_name = :svc"
                    ),
...
async def eval_evidence_expired(rule: AlertRule, ch) -> list[EvalResult]:
    ...
    async with SessionLocal() as session:
        ...
        ctrl_links = (
            await session.execute(
                select(
                    evidence_requirements.c.evidence_id,
                    evidence_requirements.c.requirement_id,
        ...
        await session.execute(
            update(Evidence)
            .where(Evidence.id.in_(expired_ids))
        ...
        for evd in rows:
            ...
            for cid in ctrl_ids:
                ...
                # Re-evaluate obligation linked to this requirement
                if ctrl.obligation_id:
                    obl = (
                        await session.execute(
                            select(Obligation).where(
                                Obligation.id == ctrl.obligation_id
                            )
                        )
                    ).scalar_one_or_none()
                    if obl is not None and obl.status not in (
                        "not_applicable",
                        "overdue",
                    ):
                        req_statuses = (
                            (
                                await session.execute(
                                    select(Requirement.status).where(
                                        Requirement.obligation_id == ctrl.obligation_id
                                    )
                                )
                            )
                            .scalars()
                        ...
                        elif all(s == "fulfilled" for s in req_statuses):
...
async def create_event(
    ...
    log.info(
        "alert.created",
        extra={"rule": rule.name, "value": value, "entity_id": entity_id},
    )
...
async def _process_single(
    ...
    if triggered and not active:
        ...
        if (
            rule.alert_type == "event"
            and rule.condition_type != "model_diverged"
            and await was_handled_recently(rule.id, entity_id)
        ):
            return
        await create_event(
            rule, value, description, entity_id, entity_type, entity_model
        )
...
async def evaluate_once() -> None:
    ...
    async with SessionLocal() as session:
        rules = (
            (await session.execute(select(AlertRule).where(AlertRule.enabled == True)))
            .scalars()
            .all()
        )
    ...
    for rule in rules:
        ...
        if not evaluator:
            log.warning(
                "policy_checker_worker.unknown_condition",
                extra={"condition": rule.condition_type},
            )
            continue
        ...
        try:
            ...
            log.exception(
                "policy_checker_worker.eval_failed", extra={"rule": rule.name}
            )
            continue
