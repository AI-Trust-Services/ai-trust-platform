...
async def get_services() -> list[dict]:
    ...
    for r in ch_rows:
        ...
        if display_name:
            ...
            logger.warning(
                "monitoring.unregistered_service",
                extra={"service_name": r["service_name"]},
            )
...
async def get_signals(
    ...
    bucket = (
        "toStartOfMinute"
        if window in ("15m", "1h")
        else "toStartOfFiveMinutes"
        if window == "6h"
        else "toStartOfTenMinutes"
    )
    ...
    if service:
        ...
        async with SessionLocal() as session:
            registered_ids = (
                (await session.execute(select(AISystem.id))).scalars().all()
            )
    ...
    logger.info(
        "monitoring.signals_fetched", extra={"service": service, "window": window}
    )
    return {
...
async def get_monitoring_stats(lifecycle: str = Query(default="")) -> dict:
    async with SessionLocal() as session:
        ...
        tier_rows = (
            await session.execute(
                select(AISystem.tier, func.count().label("n"))
                .where(lc_filter)
        ...
        type_rows = (
            await session.execute(
                select(AISystem.system_type, func.count().label("n"))
                .where(lc_filter)
        ...
        autonomy_rows = (
            await session.execute(
                select(AISystem.autonomy_level, func.count().label("n"))
                .where(lc_filter)
                .group_by(AISystem.autonomy_level)
            )
        ).all()
        ...
        compliance_by_tier_rows = (
            await session.execute(
                select(AISystem.tier, func.avg(AISystem.compliance).label("avg_c"))
                .where(lc_filter)
                .group_by(AISystem.tier)
            )
        ).all()
        ...
        below_50 = (
            await session.execute(
                ...
                .where(lc_filter, AISystem.compliance < 50)
            )
        ).scalar_one()
        ...
        open_weights_count = (
            await session.execute(
                ...
                .where(ModelCard.open_weights.is_(True))
            )
        ).scalar_one()

        buckets = {"0–20": 0, "20–40": 0, "40–60": 0, "60–80": 0, "80–100": 0}
        compliance_rows = (
            (await session.execute(select(AISystem.compliance).where(lc_filter)))
            .scalars()
            .all()
        )
