...
#
# What it does:
#   1. Applies rbac.yaml — grants the GitHub Actions OIDC identity the permissions
...
#
# NOTE: Traefik and cert-manager are provisioned automatically by Gardener shoot
# extensions — do not install them manually.
#
...
echo "==> [1/3] Applying rbac.yaml to shoot cluster"
kubectl apply -f "$SCRIPT_DIR/rbac.yaml"

echo ""
echo "==> [2/3] Requesting TLS certificate via Gardener cert-service (DNS-01, no port 80 needed)"
...
echo ""
echo "==> [3/3] Annotating Traefik LB Service for Gardener-managed DNS"
