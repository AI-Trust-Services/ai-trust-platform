...
| 4 | **Keycloak under `/keycloak`** (`KC_HTTP_RELATIVE_PATH`, `KC_HOSTNAME`, probes; keycloak-provision `KEYCLOAK_URL` + wait path; **app backends' `KEYCLOAK_URL` → `http://keycloak:8080/keycloak`** so users-backend / registry-backend token grants don't 404) | On a shoot there is ONE public host. Serving Keycloak under a path on that host avoids a second host/cert. Every component that talks to Keycloak must include the `/keycloak` context path. |
