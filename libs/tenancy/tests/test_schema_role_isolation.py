    ...
    try:
        ...
        async with engine.begin() as conn:
            for sch, role in ((schA, roleA), (schB, f"t_{b}")):
                ...
                await conn.execute(
                    text(
                        f'CREATE TABLE IF NOT EXISTS "{sch}".ai_systems '
                ...
                await conn.execute(
                    text(
                        f'GRANT SELECT,INSERT ON ALL TABLES IN SCHEMA "{sch}" TO "{role}"'
                    )
                )
            ...
            await conn.execute(
                text(f'INSERT INTO "{schB}".ai_systems VALUES (:i,:n)'),
                {"i": "SYS-B", "n": "b-secret"},
            )
            ...
            row = (
                await conn.execute(
                    text(
                        ...
                        "WHERE c.relname='ai_systems' AND n.nspname=:s"
                    ),
                    {"s": schB},
                )
            ).first()
